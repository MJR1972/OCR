...
public sealed class MainWindowViewModel : ViewModelBase
{
    ...
    private static readonly Regex TableDisplayWhitespaceRegex = new(@"\s+", RegexOptions.Compiled);
        ...
        out string emptyStateMessage)
    {
        ...
        foreach (var row in table.Rows.OrderBy(row => row.RowIndex))
        {
            ...
            foreach (var (_, valueKey, columnName) in columnMappings)
            {
                ...
                {
                    var displayText = SanitizeTableDisplayText(cell.Text);
                ...
                dataRow[columnName] = SanitizeTableDisplayText(text);
            }
    ...
    private static string SanitizeTableDisplayText(string? value)
    {
        if (string.IsNullOrWhiteSpace(value))
        {
            return "--";
        ...
        var lines = normalized
            ...
            .Select(line => TableDisplayWhitespaceRegex.Replace(line.Trim(), " "))
