...
public sealed class OcrProcessor : IOcrProcessor
{
    ...
    private static readonly Regex CollapsedWhitespaceRegex = new(@"\s+", RegexOptions.Compiled);
    ...
    private static void RefineTableCells(TesseractEngine engine, Mat processedMat, TableInfo table)
    {
        ...
        {
            var originalText = TableParsingHeuristics.CleanCellTextPreserveLines(cell.Text);
            var originalSingleLine = TableParsingHeuristics.CleanCellText(originalText);
            var expectedNumeric = IsExpectedNumericColumn(headerByCol.GetValueOrDefault(cell.ColIndex)?.Name, originalSingleLine, cell.Normalized);
            var shouldRefine = cell.IsLowConfidence || expectedNumeric || ContainsTableArtifacts(originalSingleLine);
            if (!shouldRefine)
            {
                cell.Text = originalText;
                cell.Normalized = NormalizeTableCellValue(originalText);
                continue;
            }

            var refined = RefineCellText(engine, processedMat, cell.Bbox, originalSingleLine, isHeader: false, expectedNumeric: expectedNumeric);
            var finalText = ChooseRefinedText(originalSingleLine, refined.Text, isHeader: false, expectedNumeric: expectedNumeric);
    ...
    private static string PostProcessTableCellText(string text, bool expectedNumeric)
    {
        ...
        var lines = normalized
            ...
            .Select(TableParsingHeuristics.CleanCellText)
            .Where(line => !string.IsNullOrWhiteSpace(line))
            .Select(line =>
            {
                var cleanedLine = TableArtifactRegex.Replace(line, string.Empty).Trim();
                cleanedLine = ThisIsStandaloneRegex.Replace(cleanedLine, match => ApplyTokenCasePattern("this is", match.Value));
                cleanedLine = ThisIsFollowedByWordRegex.Replace(cleanedLine, match => ApplyTokenCasePattern("this is", match.Value));
                cleanedLine = IsBeforeTestRegex.Replace(cleanedLine, match => ApplyTokenCasePattern("is ", match.Value));
                cleanedLine = Regex.Replace(cleanedLine, @"(?<=\w)#(?=\w)", " # ");
                cleanedLine = Regex.Replace(cleanedLine, @"(?<=\w)(?=#)", " ");
                cleanedLine = Regex.Replace(cleanedLine, @"(?<=#)(?=\w)", " ");
                cleanedLine = CollapsedWhitespaceRegex.Replace(cleanedLine, " ").Trim();
                return expectedNumeric ? cleanedLine.Replace(" ", string.Empty) : cleanedLine;
            })
            .Where(line => !string.IsNullOrWhiteSpace(line))
            .ToList();
    ...
    private static string ApplyTokenCasePattern(string replacement, string source)
    {
