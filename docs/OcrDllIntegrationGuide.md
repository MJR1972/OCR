...
- `PreserveInterwordSpaces` is still passed through to Tesseract, but final readable text also depends on token cleanup and reconstruction heuristics.
...
- Text cells may preserve multiple visual lines using embedded `\r\n` values in both `cells[].text` and `rows[].values`.
- Use cases:
...
- If your export pipeline expects one-line strings, normalize table cell line breaks explicitly at the integration boundary rather than assuming the DLL flattens them.
...
- For tables, prefer `rows[].values` for business export and `cells[].text`/`tokenIds` when you need visual fidelity or traceability.
