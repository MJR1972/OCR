...
- Expected Result:
  ...
  - Multiline table cell values remain multiline in the UI when the JSON contains embedded line breaks.
- Actual Result:
- Status: `Not Run`
- Notes:

### TC-F03A
- Test Case ID: `TC-F03A`
- Title: Multiline table cell preservation
- Area: Tables
- Objective: Verify table cell line breaks are preserved in JSON and in the viewer.
- Preconditions: Table selected with at least one multiline address-style or notes-style cell.
- Test Data / Input: Table document with stacked address lines inside a single cell.
- Steps:
  1. Run OCR.
  2. Open `JSON` tab and inspect `pages[].tables[].cells[].text` and `pages[].tables[].rows[].values`.
  3. Open `Tables` tab and inspect the corresponding displayed cell.
- Expected Result:
  - JSON preserves embedded `\r\n` line breaks for the multiline table cell.
  - `rows[].values` preserves the same multiline structure for string cells.
  - Tables view wraps and displays the value across multiple lines instead of forcing one continuous line.
- Actual Result:
- Status: `Not Run`
- Notes:
