...
- Line reconstruction:
  ...
  - Token cleanup repairs common OCR glue artifacts and merged-word spacing before final line text is assembled.
- Block grouping:
...
- For text-heavy cells, table cell text and row values preserve multiline content using embedded line breaks when the OCR/layout result indicates stacked lines within a cell.
- Table refinement may improve header/cell text while preserving multiline cell structure.
...
- Table string values may contain embedded `\r\n` line breaks and should not be assumed to be single-line by downstream consumers.
...
- Showcase/table viewer formatting is intentionally derived from DLL JSON rather than reconstructing table text independently in the UI.
